...
import { useCreateHotGameMutation } from '../../../../../stores/reducers/hot-games.reducer';
...
const validationSchema = z.object({
  ...
  child_game_id: z.string().optional(),
});

type CreateHotGameForm = z.infer<typeof validationSchema>;

const CreateHotGameDialog = ({ open, handleClose }: ModalProps) => {
  ...
  const form = useForm<CreateHotGameForm>({
  ...
  const [createHotGame, { isLoading: formSubmitting }] =
  ...
  const submit = async (data: CreateHotGameForm) => {
    const dataToSubmit = {
      main_game_id: data?.main_game_id,
      ...(data?.child_game_id && { child_game_id: data.child_game_id }),
    };

    if (
      childGamesByMain?.body &&
      childGamesByMain?.body?.data?.length > 0 &&
      !data?.child_game_id
    ) {
      form.setError('child_game_id', {
        message: 'Please choose one of the child games',
      });
      return;
    } else {
      try {
        const response = await createHotGame(dataToSubmit);
  ...
  return (
    <Dialog open={open}>
      <DialogContent className="w-fit min-w-[270px] md:w-[480px] max-w-[480px] py-0">
        ...
        >
          <DialogTitle> Add Hot Game </DialogTitle>
            ...
            <Form {...form}>
              ...
              >
                ...
                <Box>
                  ...
                  {form?.formState?.errors?.child_game_id && (
                ...
                <RadioGroup
                  ...
                  onValueChange={(value) => {
                    ...
                    form.setValue('child_game_id', value);
                  }}
                >
                  {childGamesByMain?.body &&
                  childGamesByMain?.body?.data?.length > 0 ? (
                    ...
                    <div className="text-xs text-surface-accent flex items-center">
                      <div className="me-1">
                        <Icons.Error />
                      </div>
                      <p className="text-xs font-semibold text-surface-accent">
                        Warning: There is no games under this main game.
                      </p>
                    </div>
                  )}
                </RadioGroup>
...
export default CreateHotGameDialog;
