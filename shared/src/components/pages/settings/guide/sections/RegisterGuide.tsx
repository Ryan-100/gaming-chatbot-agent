...
type RegisterGuideForm = z.infer<typeof validationSchema>;
...
const RegisterGuide: React.FC<RegisterGuideProps> = ({
  data,
  languageData,
}) => {
  ...
  const downDefaultData: RegisterGuideForm = {
  ...
  const form = useForm<RegisterGuideForm>({
  ...
  const submit = async (data: RegisterGuideForm) => {
    const updatedData = data.guides.map((item, index) => {
      return {
        id: item.id ?? '',
        langId: item.langId,
        guideType: GUIDE_TYPE.REGISTER,
    ...
    <Box key={localKey}>
      ...
      >
        <Box className="col-span-3">
          ...
          <Box className="col-span-3 px-[1px] border-t border-t-border-secondary pt-2">
            <FormField
              control={form.control}
          ...
          >
            {form.watch(`guides.0.showVideos`) &&
              fields.map((item, index) => (
                <Box
                  className="border border-border-secondary rounded-lg col-span-1"
                  key={index}
                >
                  <Box className="border-b border-border-secondary p-2">
                    <Flex
