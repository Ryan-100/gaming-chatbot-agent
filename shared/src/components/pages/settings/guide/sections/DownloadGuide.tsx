...
type DownloadGuideForm = z.infer<typeof validationSchema>;
...
const DownloadGuide: React.FC<DownloadGuideProps> = ({
  ...
  const downDefaultData: DownloadGuideForm = {
  ...
  const form = useForm<DownloadGuideForm>({
  ...
  const submit = async (data: DownloadGuideForm) => {
    const updatedData = data.guides.map((item, index) => {
      return {
        id: item.id ?? '',
        langId: item.langId,
        guideType: GUIDE_TYPE.DOWNLOAD,
