...
type DepositRulesFrom = z.infer<typeof validationSchema>;
...
const DepositRules: React.FC<DepositRulesProps> = ({ languageData, data }) => {
  const [enableEditMode, setEnableEditMode] = useState(false);

  const defaultData: DepositRulesFrom = {
    legals: languageData.map((item, index) => {
      return {
        ...
        legalType: LEGAL_TYPE.DEPOSIT,
      };
    }),
  };

  const form = useForm<DepositRulesFrom>({
  ...
  const submit = async (submittedData: DepositRulesFrom) => {
  ...
  return (
    <Box>
      <BotWrapper
        title="Deposit Rules"
        subTitle="Contents"
        description="The following content will be shown in Deposit Rules"
