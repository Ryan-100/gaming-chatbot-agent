...
type WithdrawRulesFrom = z.infer<typeof validationSchema>;

interface WithdrawRulesProps {
  languageData: LanguageData[];
  data: LegalListData[];
}

const WithdrawRules: React.FC<WithdrawRulesProps> = ({
  languageData,
  data,
}) => {
  const [enableEditMode, setEnableEditMode] = useState(false);

  const defaultData: WithdrawRulesFrom = {
    legals: languageData.map((item, index) => {
      return {
        ...
        legalType: LEGAL_TYPE.WITHDRAW,
      };
    }),
  };

  const form = useForm<WithdrawRulesFrom>({
  ...
  const submit = async (submittedData: WithdrawRulesFrom) => {
  ...
  return (
    <Box>
      <BotWrapper
        ...
        subTitle="Contents"
        description="The following content will be shown in Withdraw Rules"
