...
type DepositToConfirmRulesFrom = z.infer<typeof validationSchema>;

interface DepositToConfirmRulesProps {
  languageData: LanguageData[];
  data: LegalListData[];
}

const DepositToConfirmRules: React.FC<DepositToConfirmRulesProps> = ({
  languageData,
  data,
}) => {
  const [enableEditMode, setEnableEditMode] = useState(false);

  const defaultData: DepositToConfirmRulesFrom = {
    legals: languageData.map((item, index) => {
      return {
        ...
        legalType: LEGAL_TYPE.DEPOSIT_TO_CONFIRM,
      };
    }),
  };

  const form = useForm<DepositToConfirmRulesFrom>({
  ...
  const submit = async (submittedData: DepositToConfirmRulesFrom) => {
  ...
  return (
    <Box>
      <BotWrapper
        title="Deposit Rules (To Confirm)"
        subTitle="Contents"
        description="The following content will be shown in Deposit To Confirm Rules"
...
export default DepositToConfirmRules;
