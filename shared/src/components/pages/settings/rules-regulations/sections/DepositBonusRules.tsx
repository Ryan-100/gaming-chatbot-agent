...
type DepositBonusRulesFrom = z.infer<typeof validationSchema>;

interface DepositBonusRulesProps {
  languageData: LanguageData[];
  data: LegalListData[];
}

const DepositBonusRules: React.FC<DepositBonusRulesProps> = ({
  languageData,
  data,
}) => {
  const [enableEditMode, setEnableEditMode] = useState(false);

  const defaultData: DepositBonusRulesFrom = {
    legals: languageData.map((item, index) => {
      return {
        ...
        legalType: LEGAL_TYPE.DEPOSIT_BONUS,
      };
    }),
  };

  const form = useForm<DepositBonusRulesFrom>({
  ...
  const submit = async (submittedData: DepositBonusRulesFrom) => {
  ...
  return (
    <Box>
      <BotWrapper
        ...
        subTitle="Contents"
        description="The following content will be shown in Deposit Bonus Rules"
