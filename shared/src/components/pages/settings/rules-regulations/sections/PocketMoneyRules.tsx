...
type PocketMoneyRulesFrom = z.infer<typeof validationSchema>;

interface PocketMoneyRulesProps {
  languageData: LanguageData[];
  data: LegalListData[];
}

const PocketMoneyRules: React.FC<PocketMoneyRulesProps> = ({
  languageData,
  data,
}) => {
  const [enableEditMode, setEnableEditMode] = useState(false);

  const defaultData: PocketMoneyRulesFrom = {
    legals: languageData.map((item, index) => {
      return {
        ...
        legalType: LEGAL_TYPE.POCKET_MONEY,
      };
    }),
  };

  const form = useForm<PocketMoneyRulesFrom>({
  ...
  const submit = async (submittedData: PocketMoneyRulesFrom) => {
  ...
  return (
    <Box>
      <BotWrapper
        ...
        subTitle="Contents"
        description="The following content will be shown in Pocket Money Rules"
...
export default PocketMoneyRules;
