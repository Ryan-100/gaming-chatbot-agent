...
type DepositCryptoToConfirmRulesFrom = z.infer<typeof validationSchema>;

interface DepositCryptoToConfirmRulesProps {
  languageData: LanguageData[];
  data: LegalListData[];
}

const DepositCryptoToConfirmRules: React.FC<
  DepositCryptoToConfirmRulesProps
> = ({ languageData, data }) => {
  const [enableEditMode, setEnableEditMode] = useState(false);

  const defaultData: DepositCryptoToConfirmRulesFrom = {
    legals: languageData.map((item, index) => {
      return {
        ...
        legalType: LEGAL_TYPE.DEPOSIT_CRYPTO_TO_CONFIRM,
      };
    }),
  };

  const form = useForm<DepositCryptoToConfirmRulesFrom>({
  ...
  const submit = async (submittedData: DepositCryptoToConfirmRulesFrom) => {
  ...
  return (
    <Box>
      <BotWrapper
        title="Crypto Deposit Rules (To Confirm)"
        subTitle="Contents"
        description="The following content will be shown in Crypto Deposit To Confirm Rules"
...
export default DepositCryptoToConfirmRules;
