...
import { DepositBonusDetailsListData } from '../../../../../types/bonus.types';
import dayjs from '../../../../../utils/dayjs';

function Player(props: { target: DepositBonusDetailsListData }) {
  const { target } = props;
  return (
...
function DepositAmount(props: { target: DepositBonusDetailsListData }) {
  const { target } = props;
  return <Text>{CurrencyFormat(target?.depositAmount)}</Text>;
}

function BonusAmount(props: { target: DepositBonusDetailsListData }) {
  const { target } = props;
  return <Text>{CurrencyFormat(target?.bonusAmount)}</Text>;
}

export const depositBonusDetailsColumnDefs: ColumnDef<DepositBonusDetailsListData>[] =
  [
    {
      accessorKey: 'id',
      header: 'No.',
