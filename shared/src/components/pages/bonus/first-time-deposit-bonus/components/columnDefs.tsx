...
import { FirstTimeDepositBonusListData } from '../../../../../types/bonus.types';
import { isGreaterToday } from '../../components/dayChecker';
import { cn } from '../../../../../utils/cn';
import EditButton from '../../../../shared/buttons/EditButton';
import { UpdateFirstTimeDepositBonusDialog } from './UpdateFirstTimeDepositBonusDialog';
import { CurrencyFormat } from '../../../../../../src/utils/currencyFormat';

function Title(props: { target: FirstTimeDepositBonusListData }) {
  const { target } = props;
  return (
...
function StartedExpired(props: { target: FirstTimeDepositBonusListData }) {
  const { target } = props;
  return (
...
function Actions(props: { target: FirstTimeDepositBonusListData }) {
  const { target } = props;
  ...
  const handleDetailsClick = () => {
    router.push(
      `/first-time-deposit-bonus/details?name=${target?.bonusTitle}&id=${target.id}`
    );
  };
  return (
    <Flex align={'center'} className="gap-2">
      ...
      {modalOpen && (
        <UpdateFirstTimeDepositBonusDialog
          data={target}
          open={modalOpen}
...
export const columnDefs: ColumnDef<FirstTimeDepositBonusListData>[] = [
  {
