...
import {
  ...
  updateDepositBonusSchema,
} from '../../../../../types/bonus.types';
import { useGetPlayerLevelsQuery } from '../../../../../stores/reducers/player-level.reducer';
import { toast } from 'sonner';
import { useUpdateDepositBonusMutation } from '../../../../../stores/reducers/bonus.reducer';
import { PlayerLevelListData } from '../../../../../types/player-level.types';
import { isGreaterToday } from '../../components/dayChecker';
import { DeleteDepositBonusDialog } from './DeleteDepositBonusDialog';

const DATE_FORMAT = 'YYYY-MM-DD';

interface UpdateDepositBonusDialogProps {
  data: DepositBonusListData;
  open: boolean;
  onClose: () => void;
}

export const UpdateDepositBonusDialog: React.FC<
  UpdateDepositBonusDialogProps
> = ({ data, open, onClose }) => {
  ...
  const form = useForm<UpdateDepositBonusForm>({
    resolver: zodResolver(updateDepositBonusSchema),
  });
  ...
  const [updateDepositBonus] = useUpdateDepositBonusMutation();
  ...
  const submit = async (submittedData: UpdateDepositBonusForm) => {
    ...
    try {
      const response = await updateDepositBonus({
        id: data?.id ?? '',
        data: dataToSend,
      });
  ...
  return (
    <Dialog open={open}>
      <DialogContent className="py-0 w-[calc(100%-32px)] max-w-[800px] rounded-md">
        ...
        >
          <DialogTitle> Edit Deposit Bonus </DialogTitle>
