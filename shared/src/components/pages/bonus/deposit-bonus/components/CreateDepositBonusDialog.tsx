...
import {
  ...
  createDepositBonusSchema,
} from '../../../../../types/bonus.types';
import { useGetPlayerLevelsQuery } from '../../../../../stores/reducers/player-level.reducer';
import { toast } from 'sonner';
import { useCreateDepositBonusMutation } from '../../../../../stores/reducers/bonus.reducer';
import { PlayerLevelListData } from '../../../../../types/player-level.types';

const DATE_FORMAT = 'YYYY-MM-DD';

interface CreateDepositBonusDialogProps {
  open: boolean;
  onClose: () => void;
}

export const CreateDepositBonusDialog: React.FC<
  CreateDepositBonusDialogProps
> = ({ open, onClose }) => {
  const [selectedLevels, setSelectedLevels] = useState<PlayerLevelListData[]>(
    []
  );

  const form = useForm<CreateDepositBonusForm>({
    resolver: zodResolver(createDepositBonusSchema),
  });
  ...
  const [createDepositBonus] = useCreateDepositBonusMutation();

  const submit = async (submittedData: CreateDepositBonusForm) => {
    ...
    try {
      const response = await createDepositBonus(dataToSend);
  ...
  return (
    <Dialog open={open}>
      <DialogContent className="py-0 w-[calc(100%-32px)] max-w-[800px] rounded-md">
        ...
        >
          <DialogTitle> New Deposit Bonus </DialogTitle>
