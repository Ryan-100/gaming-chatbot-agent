...
import { useCreateWithdrawAdminMutation } from '../../../../../stores/reducers/withdraw-admin.reducer';
import {
  CreateWithdrawAdminForm,
  createWithdrawAdminSchema,
...
interface CreateWithdrawAdminFormDialogProps {
  open: boolean;
  onClose: (result: boolean) => void;
}

const validationSchema = createWithdrawAdminSchema
  .extend({
...
export const CreateWithdrawAdminFormDialog: React.FC<
  CreateWithdrawAdminFormDialogProps
> = ({ open, onClose }) => {
  const form = useForm<CreateWithdrawAdminForm>({
  ...
  const [createWithdrawAdmin] = useCreateWithdrawAdminMutation();
  ...
  const submit = async (data: CreateWithdrawAdminForm) => {
    const adminData: CreateWithdrawAdminForm = {
      name: data.name,
    ...
    try {
      const response = await createWithdrawAdmin(adminData);
    ...
    <Dialog open={open}>
      <DialogContent>
        <Flex justify={'between'} align={'center'}>
          <DialogTitle>Create Withdraw Admin</DialogTitle>
          <Button
            variant={'link'}
            className="p-0"
