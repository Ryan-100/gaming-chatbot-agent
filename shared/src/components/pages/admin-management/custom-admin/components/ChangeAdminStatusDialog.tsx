...
import { useChangeStatusCustomAdminMutation } from '../../../../../stores/reducers/custom-admin.reducer';
...
> = ({ id, open, onClose }) => {
  const [changeStatusCustomAdmin, { isLoading }] =
    useChangeStatusCustomAdminMutation();

  const submitHandler = async () => {
    try {
      const response = await changeStatusCustomAdmin({ id });
    ...
    <Dialog open={open}>
      <DialogContent>
        ...
        <Text className="text-secondary text-xs">
          Are you sure to change Custom Admin Status?
        </Text>
        <DialogFooter>
          <Button
            variant="link"
