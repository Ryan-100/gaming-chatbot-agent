...
const validationSchema = z.object({
  menu: z.array(
    z.object({
      ...
      file: z.object({
        id: z.string().min(10, { message: 'Image is Required' }),
        url: z.string(),
      }),
    })
  ),
  ...
  const submit = async (data: WelcomeFrom) => {
    ...
    const updatedData = data.menu.map((item, index) => {
      return {
        ...
        fileId: item.file.id,
        langId: item.langId,
        botManagementType: BOT_MANAGEMENT_TYPE.WELCOME,
        link: '',
    ...
    if (isValid) {
      try {
        ...
        if (response.data?.meta?.success) {
          toast('Successfully updated welcome message');
