...
const MainMenuAfterLogin: React.FC<WelcomeMessageProps> = ({
  languageData,
  data,
}) => {
  ...
  const submit = async (data: WelcomeFrom) => {
    ...
    const updatedData = data.menu.map((item, index) => {
      return {
        id: item.id ?? '',
        name: BOT_MANAGEMENT_TYPE.AFTER_LOGIN,
        message: item.message,
        fileId: '',
        langId: item.langId,
        botManagementType: BOT_MANAGEMENT_TYPE.AFTER_LOGIN,
        link: '',
...
export default MainMenuAfterLogin;
