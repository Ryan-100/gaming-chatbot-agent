...
const MainMenuBeforeLogin: React.FC<WelcomeMessageProps> = ({
  languageData,
  data,
}) => {
  ...
  const submit = async (data: WelcomeFrom) => {
    ...
    const updatedData = data.menu.map((item, index) => {
      return {
        id: item.id ?? '',
        name: BOT_MANAGEMENT_TYPE.BEFORE_LOGIN,
        message: item.message,
        fileId: '',
        langId: item.langId,
        botManagementType: BOT_MANAGEMENT_TYPE.BEFORE_LOGIN,
        link: '',
    ...
    if (isValid) {
      try {
        ...
        if (response.data?.meta?.success) {
          toast('Successfully updated before login');
...
export default MainMenuBeforeLogin;
