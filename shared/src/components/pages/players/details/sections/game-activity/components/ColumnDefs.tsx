...
function GameDeposit(props: { target: GameActivityByPlayerListData }) {
  const { target } = props;
  return <Text className="">{CurrencyFormat(target.game_deposit)}</Text>;
}

function GameWithdraw(props: { target: GameActivityByPlayerListData }) {
  const { target } = props;
  return <Text className="">{CurrencyFormat(target.game_withdraw)}</Text>;
}
...
export const columnDefs: ColumnDef<GameActivityByPlayerListData>[] = [
  ...
  },
  {
    header: 'Deposit',
    cell({ row }) {
      return <GameDeposit target={row.original} />;
    },
  },
  {
    header: 'Withdraw',
    cell({ row }) {
      return <GameWithdraw target={row.original} />;
    },
  },
  {
