...
import { useGetReportDepositAdminTransactionQuery } from '../../../../../../stores/reducers/report-deposit-admin.reducer';
...
export const DetailsModal = ({ id, open, onClose }: ModalProps) => {
  const { data, isLoading } = useGetReportDepositAdminTransactionQuery(id);
    ...
    <PropsTable
      rows={[
        ...
        },
        {
          key: 'Phone/ Email',
          value: `${
            DetailData?.Player?.phone ?? DetailData?.Player?.email ?? '-'
          }`,
        },
        {
          key: 'Updated Date',
