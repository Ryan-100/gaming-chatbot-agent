...
interface ImageUpdateDialogProps {
  ...
  setImage: (id: string, url: string, order: number) => void;
}

export const ImageUpdateDialog: React.FC<ImageUpdateDialogProps> = ({
  order,
  url,
  open,
  onClose,
  ...
  const [imgUrl, setImgUrl] = React.useState(url); //to show ui
...
toast.error(errorResponse?.error.data?.meta?.message);;
  ...
  const submit = async () => {
    ...
    try {
      ...
      setImage(image?.id ?? '', image?.url ?? '', order);
    } catch (error) {
      return;
    }
